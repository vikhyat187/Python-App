...
def no_of_attempt():
  '''This is the function used to get the  no of attempts based on the choice of the user but it should lie b/w 1 and 25'''
  while True:
    num=input("Enter the no of attempts do you want to choose it can range from 1 to 25 as low as the number of chances the more difficult the game would be")
    try:
      num=int(num)
      if 1<=num<=25:
        return num
      else:
        print('{0} is not in between 1 and 25 '.format(num))
    except ValueError:
      print('{0} is not a number and it should be in between 1 and 25'.format(num))
...
def get_next_letter(remaining_letters):
  '''This fn is used to get the user inputted characters'''
  if len(remaining_letters)==0:
    raise ValueError('There are no remaining_letters')
  while True:
    ...
    if len(next_letter)!=1:
      ...
      print('{0} is not in remaining_letters'.format(next_letter))
    else:
